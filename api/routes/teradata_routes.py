...
@router.post("/connect")
async def connect_to_teradata(details: models.TeradataConnectionDetails):
    try:
        schemas = teradata_helper.get_teradata_schemas(details)
...
async def list_objects(request: models.ListObjectsRequest): # Assuming a generic model for now
    try:
        objects = teradata_helper.list_teradata_objects(request.connection_details, request.schema_name, request.object_type)
...
async def extract_ddl(request: models.ExtractRequest): # Assuming a generic model for now
    ...
    try:
        ...
        ddls = teradata_helper.get_teradata_ddl(request.connection_details, request.schemas, request.object_types, request.object_names, request.select_all)
