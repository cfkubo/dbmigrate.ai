...
def poll_job_status(job_id: str, timeout: int = 180): # Increased timeout to 180 seconds
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            # Updated status checks to include 'failed' and 'failed' as non-success states
            if status in ["verified", "success"]:
                return job
            elif status in ["failed", "failed"]:
                return job # Return job even if failed to report the status
        except requests.exceptions.RequestException as e:
            print(f"Error polling job status for {job_id}: {e}")
            # Consider adding a retry mechanism here for transient network issues
...
if __name__ == "__main__":
    print("Starting Bulk Conversion Test Suite...")
    test_bulk_spf_conversion()
    test_bulk_ddl_conversion()
    print("\nBulk Conversion Test Suite Finished.")
